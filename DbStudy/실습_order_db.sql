...
-- 3. 카테고리이름, 상위카테고리이름 조회하기 = 셀프 조인으로 해결
SELECT m.menu_name, c.category_name, p.category_name as '상위카테고리 이름'
FROM tbl_menu m 
JOIN tbl_category c ON m.category_code = c.category_code
LEFT JOIN tbl_category p ON c.ref_category_code = p.category_code;
...
WHERE m.menu_price >= (SELECT AVG(menu_price) FROM tbl_menu);
