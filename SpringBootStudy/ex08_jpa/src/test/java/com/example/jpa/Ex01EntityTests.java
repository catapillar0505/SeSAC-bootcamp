...
class Ex01EntityTests {
	...
	void testEntityMapping() {
		...
		Product product = new Product("p001", "notebook", 1500000, "가볍고 높은 성능의 가성비 노트북");

		// 엔티티 매니저가 엔티티를 관리함
		// 세부 핵심 특징: 관리되는 엔티티는 반드시 ID를 가지고 있어야 한다.
		// AUTO_INCREMENT 전략을 사용하는 경우 persist() 호출 즉시 insert 쿼리를 db로 날려 id를 받아온 뒤 영속성
		// 컨텍스트에 저장한다.
		em.persist(product);

		// id 확인
		assertNotNull(product.getId()); // id가 notnull이면 테스트 통과
		System.out.println("생성된 product id:" + product.getId());
	}

	@Test
	@DisplayName("pk의 테이블 생성전략 확인")
	void entityTest() {
